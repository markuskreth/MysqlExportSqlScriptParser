...
public class Parameters {
	...
	try {
	    ...
	    if (cmd.hasOption("s")) {
		parameters = new Parameters(cmd.getOptionValue("s"),
			cmd.getOptionValue("t", createDefaultFrom(cmd.getOptionValue("s"))));
	    } else {
		printHelp(options);
		System.exit(-1);
	    }
	...
	String target = new File(new File(sourceName).getAbsoluteFile().getParent(), "parsedScript.sql")
		.getAbsolutePath();
	return target;
    }
